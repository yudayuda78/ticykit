...
<x-dashboard.app>
    <div class="mx-auto max-w-screen-xl">
        <div class="pb-6">
            <h3 class="text-lg font-bold text-gray-800">PowerPoint</h3>
        ...
        <div class="grid grid-cols-1 justify-between gap-5 pb-10 md:grid-cols-3">
            ...
            @foreach ($powerpoint as $ppt)
                <x-dashboard.items-card title="{{ $ppt->title }}"
                img="{{ asset('imgpowerpoint/' . $ppt->image) }}"
                type="PowerPoint" route="{{ route('powerpoint.download', ['id' => $ppt->id]) }}" />
            @endforeach
