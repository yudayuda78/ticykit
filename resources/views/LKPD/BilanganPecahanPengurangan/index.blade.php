...
<body>
    ...
    <form method="POST" action="{{ route('LKPD.bilanganpecahanpenguranganDownload') }}" style="display: none;">
        @csrf
    ...
    for (let i = 0; i < jumlahSoal; i++) {
        ...
        // Cari KPK penyebut
        const kpk = cariKPK(penyebutsatu, penyebutdua);

        // Sesuaikan pembilang dengan KPK
        let pembilang1Setara = pembilangsatu * (kpk / penyebutsatu);
        let pembilang2Setara = pembilangdua * (kpk / penyebutdua);

        // Hitung hasil pengurangan
        let hasilPembilang = pembilang1Setara - pembilang2Setara;

        // Pastikan hasil tidak negatif
        if (hasilPembilang < 0) {
            hasilPembilang = Math.abs(hasilPembilang);
        }

        // Untuk menyederhanakan
        const fpb = cariFPB(hasilPembilang, kpk);
        hasilPembilang /= fpb;
        const hasilPenyebut = kpk / fpb;
