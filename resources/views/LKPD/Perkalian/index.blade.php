<x-dashboard.app>
    <div class="pb-6">
        ...
        <p class="pt-2 text-sm text-gray-500">Soal-soal tentang konsep dan penerapan operasi Perkalian</p>
    </div>
    <form class="rounded-2xl border border-gray-200">
        <div class="px-7 py-5">
            <h3 class="font-bold">Informasi Lembar Kerja</h3>
            <p class="text-sm text-gray-500">Ketikan informasi tentang LKPD operasi Perkalian yang akan dibuat</p>
        </div>
        <hr />
        <div class="space-y-3 p-7">
            <x-forms.input id="judul" label="Judul" name="judul" placeholder="Cth: Latihan Soal Perkalian"
                required />
    ...
    <form method="POST" action="{{ route('LKPD.perkaliandownload') }}" style="display: none;">
        @csrf
    ...
    <script>
        ...
        document.getElementById('kirimBtn').addEventListener('click', function() {
            ...
            for (let i = 0; i < jumlahSoal; i++) {
                ...
                let soalHTML =
                    // `<p>${i + 1}) ${angkaPertama} x ${angkaKedua} = <span class="hasilHide">${hasil}</span></p>`;
                    `<p class="hidden">${i + 1}) ${angkaPertama} x ${angkaKedua} = <span class="hasilHide">${hasil}</span></p> <div class="flex"><div class="font-bold">${i + 1})</div><div class="ml-4 w-full"><div class="text-right">${angkaPertama}</div><div class="text-right">${angkaKedua}</div><hr class="my-2 border-t border-black" /><div class="text-right"><span class="hasilHide">${hasil}</span></div></div><span class="ml-1 mt-10 font-bold">x</span></div>`;
        ...
        document.querySelector('.download').addEventListener('click', function() {
            ...
            soalElements.forEach((soal) => {
                ...
                let match = soal.textContent.match(/(\d+)\s\x\s(\d+)/);
                if (match) {
            ...
            document.querySelector('form[action="{{ route('LKPD.perkaliandownload') }}"]').submit();
        });
    </script>
</x-dashboard.app>
{{-- <!DOCTYPE html>
...
<body>
    ...
    <div class="mb-4">
        ...
        <input type="number" name="jumlahSoal" id="jumlahSoal" class="mt-1 p-2 w-full border border-gray-300 rounded-md" max="30" required>
    </div>
    ...
    <script>
        ...
        document.getElementById('kirimBtn').addEventListener('click', function() {
            ...
            for (let i = 0; i < jumlahSoal; i++) {
                ...
                let soalHTML = `<p>${i + 1}) ${angkaPertama} x ${angkaKedua} = <span class="hasilHide">${hasil}</span></p>`;
                hasilDiv.innerHTML += soalHTML;
            }
        });
